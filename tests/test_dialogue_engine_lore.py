...
def test_birthday_lore_reaches_prompt_result_memory_and_log():
    ...
    assert "birthday_orange_cake" in result.lore_fragments_used
    assert model.calls
    assert "fragment_id: birthday_orange_cake" in model.calls[0]
...
def test_bubble_lore_reaches_prompt_without_deep_void():
    pack = load_character("daniya")
    model = RecordingModel()
    ...
    assert "deep_void" not in model.calls[0]
...
def test_return_date_special_response_records_goodbye_lore_without_model_call():
    ...
    assert result.lore_fragments_used == ["goodbye_name"]
    assert model.calls == []
    assert load_event_log()[-1]["lore_fragments_used"] == ["goodbye_name"]
...
def test_explicit_l4_question_adds_only_trimmed_deep_void():
    pack = load_character("daniya")
    model = RecordingModel()
    ...
    assert "deep_void" in result.lore_fragments_used
    assert "fragment_id: deep_void" in model.calls[0]
    assert pack.lore not in model.calls[0]
