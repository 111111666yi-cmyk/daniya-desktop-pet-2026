...
def test_birthday_keyword_retrieves_trimmed_l2_fragment():
    ...
    assert fragment["id"] == "birthday_orange_cake"
...
def test_l4_only_when_explicit_story_question():
    ...
    assert "deep_void" not in [fragment["id"] for fragment in plain]
    ...
    assert "deep_void" in [fragment["id"] for fragment in explicit]
    deep = next(fragment for fragment in explicit if fragment["id"] == "deep_void")
