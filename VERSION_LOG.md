...
## v0.83 — 对话级长期记忆与观察日记 (2026-06-12)

- 新增默认关闭的对话级长期记忆；每次完成对话后在本机保存用户与达妮娅的成对记录。
- 使用本地稀疏词向量和余弦相似度检索，只把当前问题最相关的少量历史记录注入统一文本 Provider prompt。
- 设置中心新增“记忆与日记”页，可查看原始可见内容、调整 top-k/容量并单独清空。
- API Key、Token、密码和密钥样式文本不会写入长期记忆；所有记录位于 ignored `data/`，不进入 Git 或发布包。
- 观察日记从最近 1-30 天事件日志构建事实摘要，发送前必须二次确认；Provider 未真实返回时不保存 fallback 文本。
